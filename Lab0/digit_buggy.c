...
int numdigits (int n) {
    int count = 1; // ADDED: initialized count to 1 
    while ((n /= 10) != 0) {
        count++;
    }
        return count;
...
int digit_place (int d, int tensplace) {
    ...
    // return d / (p10 * 10) % p10;
    return d % (p10 * 10) / p10; // ADDED: Changed order ot the / and %
}
// int digit_place(int d, int tensplace) {
//  int p10 = powerof10(tensplace);
...
int main (int argc, char **argv) {
    char *digit_str[] = { "zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
